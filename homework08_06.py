...
class Xerox(OfficeEquipment):
    def __init__(self, **kwargs):
        super().__init__(eq_type='Копировальный аппарат', **kwargs)
...
if __name__ == '__main__':
    ...
    for idx, itm in storage.filter_by(department=None, vendor='OKI', model='5367-AD'):
        print(f'Резервируем {itm} в "Отдел К".')
        storage.transfer(idx, 'Отдел К')

    for idx, itm in storage.filter_by(department=None):
        print(idx, f'{itm} не закреплен за отделом.')
